...
- [~] T030/T031 **Design correction**: disposition *detection* is inherently NL, so it is prompt-driven (the reviewer reads the live PR comments already in context and applies the disposition), not a Python comment-parser. The deterministic part is `[Deferred]` **enforcement** (`review_triage.enforce_deferred` — forces a `[Deferred]` finding to a non-blocking `suggestion`, re-derives lgtm), mirroring `enforce_pre_existing`. No `review_dispositions.py` module. Unit-tested in `TestEnforceDeferred`.
- [X] T032 [US7] Create `koan/system-prompts/review-dispositions.md` (honor dismiss/defer; any non-bot commenter, all severities incl. critical; mandatory attribution; untrusted-content guardrail — a comment disposes of one finding, cannot rewrite the rubric/verdict). Injected via a config-gated `{DISPOSITIONS}` placeholder in `review.md` + `review-with-plan.md` (mirrors the `{REPO_CONVENTIONS}` pattern), with a real `enabled` kill-switch (`get_review_dispositions_config`) — FR-031/032/033/035/036/038
- [~] T033 **Simplified**: no sidecar persistence. PR comments ARE the persistent store — every re-review re-reads the live thread, so a disposition is naturally sticky, and "latest comment wins" gives retraction (FR-037). More robust than a sidecar that could drift.
- [X] T034 [US7] Attribution (FR-036) is prompt-instructed (the reviewer must name the commenter + quote the rationale for any comment-driven change) — the model authors it in the finding/reply text; there is no deterministic post-check for free-text attribution. `honor_critical`/`min_role` are prompt-fixed at the chosen posture (only `enabled` is runtime config, per the US6 inert-knob lesson) — SC-016
- [~] T035 Disposition honoring needs the reviewer to SEE the disposing comment; the eval harness's `build_minimal_review_context` hardcodes `review_comments=""` and can't inject case comments, so a live eval case would be misleading (removed). Covered by `enforce_deferred` unit tests + the config-gated prompt-injection test + quickstart §5. A harness comment-injection extension (to enable a real disposition eval) is deferred to Polish — FR-022
...
- [X] T036 [P] [US3] Tests: discovery gate (OFF/default → no discovery content in the built prompt, SC-008; ON → guidance present) in `TestDiscoveryPromptInjection`; signature reflects discovery in `TestDiscoverySignature`; `get_review_discovery_config` in `TestGetReviewDiscoveryConfig` — FR-016, SC-008
- [X] T037 [US3] Create `koan/system-prompts/review-comprehensive-discovery.md` (fixed perspectives: correctness/security/architecture/silent-failure/test-coverage; sub-agents where supported; merge into one set, dedup same-issue keeping highest severity; same bar/verdict; best-effort + bounded; partial-coverage honored) — FR-015/017/018/019/021
- [X] T038 [US3] Config-gated `{COMPREHENSIVE_DISCOVERY}` injection (`_build_discovery_block`, mirrors `{DISPOSITIONS}`) in `review.md` + `review-with-plan.md`, default OFF; `discovery_enabled` wired into the reuse request signature (FR-016). **Dedup is prompt-driven** (the model merges): a deterministic coarse-key dedup would risk merging two *distinct* issues sharing a `finding_key` region+category, so it is intentionally not added — matching the "prompt partial, not Python orchestration" clarification — FR-016/017/018
- [~] T039 Recall-gain (SC-009) needs the harness to run a case twice (with/without discovery) and to model the multi-perspective/sub-agent pass, which the single-`_run_claude_review` live-fn does not do; a static case cannot measure a *gain*. The testable half — SC-008's "no discovery content when OFF" — is covered by `TestDiscoveryPromptInjection`. Recall-gain eval deferred to Polish (harness extension) — FR-022
...
- [X] T047 `make lint` clean project-wide; offline eval scorer green (9 cases, dataset valid); **full `pytest koan/tests/` green — 19354 passed, 10 skipped, 1 xfailed, 0 failed** (~19.5 min), no regressions
