...
def fetch_github_issues(refs: List[Tuple[str, str, int]]) -> str:
    ...
    # Use run_gh (not raw subprocess) so each fetch gets retry-with-backoff for
    # transient errors, SSOAuthRequired mapping, and GIT_OPERATION audit logging.
    ...
    for owner, repo, number in refs:
        ...
        try:
            ...
            # Best-effort contract: a failed fetch (non-zero exit, SSO auth,
            # missing gh, timeout) skips the ref rather than aborting the review.
