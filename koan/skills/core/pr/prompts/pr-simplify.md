# Simplify Pass — Post-Review Readability

You are performing a readability-only simplification pass on the files changed by
this branch.

Working directory: `{PROJECT_PATH}`

## Your Task

1. Run `git diff --name-only origin/{BASE}...HEAD` to identify the files this branch
   changed since it diverged from the upstream target branch. The three-dot form
   diffs against the merge-base, so only this branch's own work is included.
2. Read each changed file and look for **readability issues only**:
   - Unclear variable or function names that could be more descriptive
   - Nested ternary operators (replace with if/else chains)
   - Unnecessary comments that describe what the code obviously does
   - Dead code branches that can never execute
   - Boilerplate in the changeset that duplicates logic already available in an
     existing helper — replace it with a call to that helper
   - Opportunities to reuse an existing helper instead of reimplementing similar logic
   - Cases where adding a parameter to an existing helper would remove duplication —
     prefer extending the existing helper over introducing a new duplicate one
3. Apply **readability-only fixes** — each change must make the code clearer without
   changing behavior.

Output a brief summary of what you simplified (or "No simplifications needed" if clean).
