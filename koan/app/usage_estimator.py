...
def parse_claude_usage_output(text: str) -> Optional[dict]:
    """Parse ``claude usage`` CLI output to extract real API quota percentages.

    Expected format (with progress bars and ANSI codes stripped)::
    ...
    Returns:
        dict with keys: session_pct (int), weekly_pct (int),
        session_reset (str), weekly_reset (str).
    ...
    # Parse sections by looking for headers then extracting % and reset
    # Session: "Current session" section
    session_match = re.search(
        r'Current\s+session\b.*?(\d+)%\s*used.*?Resets?\s+(.+?)(?:\n|$)',
        clean, re.DOTALL | re.IGNORECASE,
    )
    # Weekly: "Current week (all models)" section
    weekly_match = re.search(
        r'Current\s+week\s*\(all\s+models\).*?(\d+)%\s*used.*?Resets?\s+(.+?)(?:\n|$)',
        clean, re.DOTALL | re.IGNORECASE,
    )
    ...
    return {
        ...
        "weekly_pct": int(weekly_match.group(1)),
        "session_reset": session_match.group(2).strip(),
        "weekly_reset": weekly_match.group(2).strip(),
    }


def cmd_refresh_from_cli(usage_md: Path, timeout: int = 15) -> bool:
    """Refresh usage.md by running ``claude usage`` and parsing real API percentages.

    This produces accurate usage data directly from the Anthropic API,
    unlike the internal token-accumulation estimates.

    Returns:
        True if usage.md was successfully updated, False on any failure.
    """
    try:
        ...
        print(f"[usage_estimator] claude usage failed: {e}", file=sys.stderr)
        return False

    parsed = parse_claude_usage_output(combined)
    if parsed is None:
        print("[usage_estimator] Could not parse claude usage output", file=sys.stderr)
        return False

    # Write usage.md in the format expected by usage_tracker.py
    now = datetime.now()
    content = f"""# Usage (from claude CLI)

Session (5hr) : {parsed['session_pct']}% (reset in {parsed['session_reset']})
Weekly (7 day) : {parsed['weekly_pct']}% (Resets in {parsed['weekly_reset']})

<!-- Auto-generated by usage_estimator.py from claude usage — {now.strftime('%Y-%m-%d %H:%M')} -->
"""
    atomic_write(usage_md, content)
    return True
