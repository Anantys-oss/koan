"""Tests for commit_tracker.py — Kōan self-commit tracking across startups."""
...
class TestStatePersistence:
    ...
    def test_save_overwrites(self, tmp_path):
        _save_state(str(tmp_path), {"koan": "aaa"})
        _save_state(str(tmp_path), {"koan": "bbb"})
        loaded = _load_state(str(tmp_path))
...
class TestGetLog:
    def test_returns_lines_and_count(self):
        ...
        with patch("app.commit_tracker.run_git", return_value=(0, log_output, "")):
            lines, total = _get_log("/koan", "old_sha", limit=3)
            assert len(lines) == 3
            assert total == 5
    ...
    def test_returns_all_when_under_limit(self):
        ...
        with patch("app.commit_tracker.run_git", return_value=(0, log_output, "")):
            lines, total = _get_log("/koan", "old_sha")
            assert len(lines) == 2
            assert total == 2
...
class TestRecordAndReport:
    ...
    def test_no_change_returns_none(self, tmp_path):
        ...
        _save_state(str(tmp_path), {"koan": "abc123"})
    ...
    def test_changed_head_reports_commits(self, tmp_path):
        ...
        _save_state(str(tmp_path), {"koan": "oldsha111"})
        ...
        with patch("app.commit_tracker.run_git", side_effect=mock_run_git):
            msg = record_and_report("/koan", str(tmp_path))
    ...
    def test_changed_head_truncates_long_log(self, tmp_path):
        ...
        _save_state(str(tmp_path), {"koan": "oldsha"})
    ...
    def test_head_unreadable_returns_none(self, tmp_path):
        ...
        with patch("app.commit_tracker.run_git", return_value=(1, "", "fatal")):
            msg = record_and_report("/koan", str(tmp_path))
        assert msg is None
        assert _load_state(str(tmp_path))["koan"] == "oldsha"
    ...
    def test_preserves_other_keys_in_state(self, tmp_path):
        ...
        _save_state(str(tmp_path), {"koan": "old", "other_key": "keep"})
        with patch("app.commit_tracker.run_git", return_value=(0, "new\n", "")):
