...
- **Adding a new core skill** — Every core skill requires ALL of the following. Missing any step leaves the skill broken or undiscoverable:
  ...
  6. **Evals (if LLM-driven with a checkable output contract)**: If the skill invokes the LLM and emits a checkable structured output (valid JSON / a parseable contract), add golden-dataset evals — a scorer (`score_<skill>`), `koan/skills/core/<name>/evals/cases/*.json`, and a `baseline.json` stub — registering the scorer in `SCORERS`. Reuse the skill's own validator/parser as the single source of truth (the way `review` reuses `validate_review`). Orchestration/queue skills with no structured LLM output are **exempt** — document the skill in `EVAL_EXEMPT_SKILLS` instead of fabricating a dataset. See `koan/app/skill_evals.py`, `specs/components/skills.md` → "Skill evaluation harness", and `docs/operations/skill-evals.md`.
