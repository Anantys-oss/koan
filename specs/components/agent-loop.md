...
| `stagnation_monitor` | Daemon thread hashing last-N stdout lines; kills the subprocess group after K identical hashes; requeues up to `max_retry_on_stagnation`. Retry count/pattern is exposed via `get_retry_info()` and consumed by `prompt_builder._get_stagnation_retry_section()` to coach the requeued session away from repeating the same stuck pattern. |
