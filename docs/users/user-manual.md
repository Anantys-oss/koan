...
- **The review shell:** reviews can run read-only inspection commands (`git log`, `git blame`, `git grep`, `rg`, `sed -n '10,40p'`, `find`, …) so a finding can be checked against history and real callers instead of guessed from the diff. Every command is adjudicated by a hook before it runs: one simple command per call, no pipes, no redirection, no chaining, no command substitution, and no writer, network client, or interpreter. A rejected command is refused, never silently rewritten.
