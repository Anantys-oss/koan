...
| `/review <PR> [PR ...] [--bot-comments]` | `/rv`, `/rereview`, `/re_review` | Review one or more pull requests; each URL queues a separate review mission. Reviews are read-only — shell, write, and edit tools are denied at the CLI, not merely left unrequested, and a provider that cannot enforce that is refused rather than run unrestricted. `--bot-comments` triages bot findings | Yes |
