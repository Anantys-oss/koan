...
**Draft-PR review gate (opt-in):** By default Kōan reviews a PR as soon as it is attached as a reviewer, whether or not the PR is a draft. Set `review_draft_skip.enabled: true` to defer that automatic review while a PR is in draft state — the author has marked it not-ready, so the bot waits. The reliable way to resume a deferred review is to send `/review` when you want it — that always works. It may **also** resume automatically once the PR is marked ready for review: because the deferral writes no dedup or cooldown state, any re-surfacing of the same review request is re-evaluated fresh (with the PR no longer a draft) and reviewed on the next poll. Whether GitHub re-surfaces the request on the draft→ready transition is GitHub-side behavior, so treat `/review` as the primary remedy. This gates **only** the "bot attached as reviewer" path: an explicit `/review` from chat or a GitHub `@mention` is always honored immediately, regardless of this flag or the PR's draft state. A deferred review sends one info message so you know it was intentionally postponed (not dropped).
