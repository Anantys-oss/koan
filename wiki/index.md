...
- [`architecture/prompt-context-budget.md`](docs/architecture/prompt-context-budget.md) — Measures what every mission prompt costs (fixed `agent.md` base, conditional sections, per-skill prompt sizes) and separates operator-tunable inputs from repo-frozen prompt artifacts.
