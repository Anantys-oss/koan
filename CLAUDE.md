...
- **Never commit `instance/`; keep `instance.example/` skill-free** — `instance/` is a **private, gitignored** runtime tree (missions, memory, config), hydrated from `instance.example/` at setup. Never `git add -f` anything under `instance/`; a committed file there leaks an operator's private state forever. And never add skills to `instance.example/` — that directory is the *fresh-instance template* and must stay skill-free; core skills live in `koan/skills/`, custom ones in a private `instance/skills/`. CI (`.github/workflows/instance-guard.yml` → `scripts/instance_guard.py`) fails a PR that tracks either; run `python3 scripts/instance_guard.py` locally to check.
...
specs/011-config-mission-hooks/plan.md
<!-- SPECKIT END -->
