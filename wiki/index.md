...
- [`004-mission-store/spec.md`](../specs/004-mission-store/spec.md) — **draft** (design only, no `tasks.md`, `Status: Draft`). Migrate missions to a single SQLite store in one shot, behind a `MissionStore` port; ship only the `sqlite` adapter in-tree (an out-of-tree file or other store is loadable via config). Supersedes the #2209 mirror; requires a blocking, likely-MAJOR Constitution III amendment (missions leave files). Also requires command/dashboard/API visibility for done/failed history (replacing the `missions.md` window). PR 1 of 2 is spec-only.
