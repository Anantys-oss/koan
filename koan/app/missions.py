...
_CANONICAL_KEY_STRIP_RE = re.compile(
    ...
    r"|\s*\[decompose\]"            # [decompose] user request tag
    r"|\s*\[group:[^\]]*\]"         # [group:ID] sub-mission tag
    r"|\s*\[decomposed:[^\]]*\]"    # [decomposed:ID] parent tag
)
...
def extract_next_pending(content: str, project_name: str = "") -> str:
    ...
    while i < len(lines):
        ...
        # Skip decomposed parents — they stay in Pending as a marker while
        # their [group:ID] sub-missions run, and must never be picked for
        # execution (the group-completion sweep transitions them out).
...
# --- Mission decomposition tags -------------------------------------------
#
# Decomposition uses three tags, all applied automatically:
...
#   [decomposed:ID]    — the original parent, kept in Pending but skipped by the
#                        picker while its sub-tasks run
# IDs are short alphanumeric hashes generated by the iteration manager.
_GROUP_TAG_RE = re.compile(r'\[group:([A-Za-z0-9]+)\]', re.IGNORECASE)
_DECOMPOSED_TAG_RE = re.compile(r'\[decomposed:([A-Za-z0-9]+)\]', re.IGNORECASE)


def extract_group_tag(text: str) -> str:
    """Return the group ID from a [group:ID] tag, or "" if absent."""
    match = _GROUP_TAG_RE.search(text or "")
    return match.group(1) if match else ""


def extract_decomposed_tag(text: str) -> str:
    """Return the group ID from a [decomposed:ID] tag, or "" if absent."""
    match = _DECOMPOSED_TAG_RE.search(text or "")
    return match.group(1) if match else ""
...
def inject_subtasks(
    ...
    """Insert decomposed sub-missions into Pending, in order.

    Each sub-task becomes a fresh ``- ...`` Pending line tagged ``[group:ID]``
    (and inheriting the parent's ``[project:X]`` tag, if any), stamped with a
    queued timestamp. Sub-tasks are inserted immediately after the parent line
    so they are picked in order.

    Returns content unchanged if the parent line is not found in Pending or the
    sub-task list is empty (after filtering blanks) — callers detect the no-op
    by comparing the returned content to the input.
    """
    ...
    for i, raw_line in enumerate(lines):
        ...
        if stripped.startswith("##") and not stripped.startswith("###"):
            section_name = stripped.lstrip("#").strip().lower()
            in_pending = _SECTION_MAP.get(section_name, "") == "pending"
            continue
        if not in_pending:
            continue
...
def mark_parent_decomposed(
    ...
    """Append a [decomposed:ID] tag to the parent mission line in Pending.

    The parent stays in Pending as a marker; the picker skips it (see
    :func:`extract_next_pending`) until the group-completion sweep transitions
    it to Done/Failed. The tag is inserted before any timestamp markers.

    Returns content unchanged if the parent is not found or is already tagged —
    callers detect the no-op by comparing returned content to the input.
    """
    search_key = parent_text.splitlines()[0].strip() if parent_text else ""
    ...
    for i, raw_line in enumerate(lines):
        ...
        if (
            ...
            ts_match = re.search(r'\s*[⏳▶]\(', base)
            if ts_match:
                new_line = base[:ts_match.start()] + f" {tag}" + base[ts_match.start():]
            else:
                new_line = f"{base} {tag}"
            ending = raw_line[len(raw_line.rstrip("\r\n")):]
            lines[i] = new_line + (ending or "\n")
...
def find_decomposed_parents_ready(content: str) -> List[str]:
    """Return [decomposed:ID] parents in Pending with no active sub-missions.

    A parent is "ready" once none of its ``[group:ID]`` sub-missions remain in
    Pending or In Progress (they have all reached Done or Failed). Each returned
    element is the full parent block text.
    """
    sections = parse_sections(content)
    active_items = sections["pending"] + sections["in_progress"]
    ready: List[str] = []
    for item in sections["pending"]:
        gid = extract_decomposed_tag(item)
        if not gid:
            continue
        still_active = any(extract_group_tag(other) == gid for other in active_items)
        if not still_active:
            ready.append(item)
    return ready
...
def check_all_subtasks_failed(content: str, group_id: str) -> bool:
    """Return True if every finished sub-task of a group failed.

    True when at least one ``[group:ID]`` sub-mission is in Failed and none is
    in Done. Used by the sweep to decide whether a ready parent should be
    auto-failed (all sub-tasks failed) rather than auto-completed.
    """
    if not group_id:
        return False
    sections = parse_sections(content)
    n_failed = sum(1 for it in sections["failed"] if extract_group_tag(it) == group_id)
    n_done = sum(1 for it in sections["done"] if extract_group_tag(it) == group_id)
    return n_failed > 0 and n_done == 0
