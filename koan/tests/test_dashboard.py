...
def app_client(instance_dir, tmp_path):
    ...
    with patch.object(dashboard.state, "INSTANCE_DIR", instance_dir), \
         ...
         patch.object(dashboard.state, "RECURRING_FILE", instance_dir / "recurring.json"), \
...
class TestSignals:
    def test_no_signals(self, tmp_path):
        with patch.object(dashboard.state, "KOAN_ROOT", tmp_path):
            status = dashboard_service.stats.get_signal_status()
...
class TestJournal:
    def test_get_entries(self, instance_dir):
        with patch.object(dashboard.state, "JOURNAL_DIR", instance_dir / "journal"):
            entries = dashboard_service.journal.get_journal_entries(limit=7)
            assert len(entries) == 1
            assert entries[0]["date"] == "2026-02-01"
            assert entries[0]["entries"][0]["project"] == "koan"

    def test_empty_journal(self, tmp_path):
        with patch.object(dashboard.state, "JOURNAL_DIR", tmp_path / "journal"):
            entries = dashboard_service.journal.get_journal_entries()
            assert entries == []
...
class TestBuildDashboardPrompt:
    ...
    def test_lite_prompt_strips_journal_and_summary(self, instance_dir):
        with patch.object(dashboard.state, "CONVERSATION_HISTORY_FILE", instance_dir / "history.jsonl"), \
             patch.object(dashboard.state, "SOUL_FILE", instance_dir / "soul.md"), \
             patch.object(dashboard.state, "SUMMARY_FILE", instance_dir / "memory" / "summary.md"), \
             patch.object(dashboard.state, "INSTANCE_DIR", instance_dir), \
             patch("app.dashboard.chat.load_recent_history", return_value=[]), \
             patch("app.dashboard.chat.format_conversation_history", return_value=""), \
             patch("app.dashboard.chat.get_tools_description", return_value=""):
            prompt = dashboard.chat._build_dashboard_prompt("hello", lite=True)
...
class TestParseProject:
    def test_english_tag(self):
        project, text = dashboard.chat.parse_project("[project:koan] fix bug")
        assert project == "koan"
        assert text == "fix bug"

    def test_french_tag(self):
        project, text = dashboard.chat.parse_project("[projet:koan] fix bug")
        assert project == "koan"
        assert text == "fix bug"

    def test_no_tag(self):
        project, text = dashboard.chat.parse_project("fix bug")
...
class TestApiProgressStream:
    def test_stream_returns_sse_content_type(self, app_client):
        ...
        with dashboard.app.test_request_context("/api/progress/stream"):
            resp = dashboard.chat.api_progress_stream()
...
class TestJournalEntries:
    ...
    def test_flat_journal_file(self, tmp_path):
        ...
        with patch.object(dashboard.state, "JOURNAL_DIR", journal):
            entries = dashboard_service.journal.get_journal_entries()
            assert len(entries) == 1
    ...
    def test_limit_parameter(self, tmp_path):
        ...
        with patch.object(dashboard.state, "JOURNAL_DIR", journal):
            entries = dashboard_service.journal.get_journal_entries(limit=3)
...
class TestReadFile:
    def test_existing_file(self, tmp_path):
        ...
        assert dashboard_service.read_file(f) == "hello world"

    def test_missing_file(self, tmp_path):
        assert dashboard_service.read_file(tmp_path / "missing.md") == ""
...
class TestBuildDashboardPromptFull:
    ...
    def test_full_prompt_includes_journal_and_summary(self, instance_dir):
        with patch.object(dashboard.state, "CONVERSATION_HISTORY_FILE", instance_dir / "history.jsonl"), \
             ...
             patch("app.dashboard.chat.format_conversation_history", return_value=""), \
             patch("app.dashboard.chat.get_tools_description", return_value=""):
            prompt = dashboard.chat._build_dashboard_prompt("hello", lite=False)
        assert "You are Kōan" in prompt
    ...
    def test_full_prompt_truncates_summary(self, instance_dir):
        ...
        with patch.object(dashboard.state, "CONVERSATION_HISTORY_FILE", instance_dir / "history.jsonl"), \
             ...
             patch("app.dashboard.chat.format_conversation_history", return_value=""), \
             patch("app.dashboard.chat.get_tools_description", return_value=""):
            prompt = dashboard.chat._build_dashboard_prompt("hello")
...
class TestProjectFiltering:
    ...
    def test_api_projects(self, app_client):
        with patch("app.dashboard_service.missions.get_known_projects", return_value=[("koan", "/p/koan")]):
    ...
    def test_api_projects_includes_mission_tags(self, app_client, instance_dir):
        ...
        with patch("app.dashboard_service.missions.get_known_projects", return_value=[]):
            resp = app_client.get("/api/projects")
        data = resp.get_json()
    ...
    def test_filter_missions_helper(self):
        ...
        filtered = dashboard_service.missions.filter_missions_by_project(missions, "koan")
    ...
    def test_filter_missions_empty_project(self):
        ...
        filtered = dashboard_service.missions.filter_missions_by_project(missions, "")
        assert filtered == missions

    def test_project_badge_filter(self):
        assert "koan" in dashboard._helpers.project_badge_filter("- [project:koan] Fix bug")
        assert dashboard._helpers.project_badge_filter("- Fix bug") == ""

    def test_strip_project_tag_filter(self):
        assert dashboard._helpers.strip_project_tag_filter("- [project:koan] Fix bug") == "- Fix bug"
        assert dashboard._helpers.strip_project_tag_filter("- Fix bug") == "- Fix bug"

    def test_linkify_filter_converts_urls(self):
        result = dashboard._helpers.linkify_filter("Fix https://github.com/org/repo/pull/42 now")
        assert 'href="https://github.com/org/repo/pull/42"' in result
        assert 'target="_blank"' in result
        assert 'rel="noopener noreferrer"' in result

    def test_linkify_filter_no_url(self):
        result = dashboard._helpers.linkify_filter("Fix the bug")
        assert "<a " not in result
        assert result == "Fix the bug"

    def test_linkify_filter_escapes_html(self):
        result = dashboard._helpers.linkify_filter("<script>alert(1)</script>")
        assert "<script>" not in result
        assert "&lt;script&gt;" in result

    def test_linkify_filter_multiple_urls(self):
        result = dashboard._helpers.linkify_filter("See https://jira.example.com/PROJ-123 and https://github.com/org/repo/issues/5")
        assert result.count("<a ") == 2

    def test_linkify_shortens_github_issue_url(self):
        result = dashboard._helpers.linkify_filter("Fix https://github.com/org/repo/issues/123 done")
    ...
    def test_linkify_shortens_github_pull_url(self):
        result = dashboard._helpers.linkify_filter("See https://github.com/org/repo/pull/42")
        assert 'href="https://github.com/org/repo/pull/42"' in result
        assert ">#42</a>" in result

    def test_linkify_shortens_jira_url(self):
        result = dashboard._helpers.linkify_filter("Fix https://jira.example.com/browse/PROJ-456 now")
    ...
    def test_linkify_preserves_plain_url(self):
        result = dashboard._helpers.linkify_filter("See https://example.com/docs")
...
"""
    ...
    def test_extracts_phases(self):
        result = dashboard_service.plans.parse_plan_progress(self._STANDARD_PLAN)
    ...
    def test_detects_checkmark_completion(self):
        result = dashboard_service.plans.parse_plan_progress(self._COMPLETED_PHASE)
    ...
    def test_detects_checkbox_completion(self):
        result = dashboard_service.plans.parse_plan_progress(self._CHECKBOX_PHASE)
        assert result["phases"][0]["completed"] is True

    def test_detects_done_text_completion(self):
        result = dashboard_service.plans.parse_plan_progress(self._DONE_TEXT_PHASE)
        assert result["phases"][0]["completed"] is True
...
class TestPlansPage:
    ...
    def test_api_plans_skips_projects_without_github_url(self, app_client):
        with patch("app.utils.get_known_projects", return_value=[("myproject", "/some/path")]), \
             patch("app.dashboard_service.plans.get_project_repo", return_value=None):
            resp = app_client.get("/api/plans")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["plans"] == []
    ...
    def test_api_plans_returns_plan_issues(self, app_client):
        ...
        with patch("app.utils.get_known_projects", return_value=[("myproject", "/path")]), \
             patch("app.dashboard_service.plans.get_project_repo", return_value="owner/repo"), \
             patch.dict("app.dashboard.state._plans_cache", {}, clear=True), \
    ...
    def test_api_plans_force_refresh_bypasses_cache(self, app_client):
        ...
        with patch("app.utils.get_known_projects", return_value=[("myproject", "/p")]), \
             patch("app.dashboard_service.plans.get_project_repo", return_value="owner/repo"), \
             patch.dict("app.dashboard.state._plans_cache", fresh_cache, clear=True), \
    ...
    def test_api_plan_detail_no_github_url(self, app_client):
        with patch("app.dashboard_service.plans.get_project_repo", return_value=None):
    ...
    def test_api_plan_detail_returns_structure(self, app_client):
        ...
        with patch("app.dashboard_service.plans.get_project_repo", return_value="owner/repo"), \
             patch("app.github.run_gh", return_value=gh_response), \
             patch.object(dashboard.state, "MISSIONS_FILE", Path("/nonexistent/missions.md")):
            resp = app_client.get("/api/plans/myproject/42")
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_find_linked_missions(self, instance_dir):
        ...
        with patch.object(dashboard.state, "MISSIONS_FILE", missions_file):
            linked = dashboard_service.plans.find_linked_missions(
    ...
    def test_get_project_repo_loads_config_dict(self):
        ...
        with patch("app.projects_config.load_projects_config", return_value=fake_cfg), \
             patch.object(dashboard.state, "KOAN_ROOT", Path("/fake/root")):
            result = dashboard_service.plans.get_project_repo("myproj")
...
class TestApiHealth:
    ...
    def test_api_health_disk_warn(self, app_client, tmp_path):
        ...
        with patch.object(dashboard.state, "KOAN_ROOT", tmp_path), \
             patch("app.dashboard.core.shutil.disk_usage",
...
class TestAgentControls:
    ...
    def test_pause_with_duration(self, app_client, tmp_path):
        ...
        with patch("app.dashboard.prs.time.time", return_value=fixed_time):
...
class TestPassphraseGate:
    ...
    def test_no_passphrase_allows_access(self, app_client):
        with patch.object(dashboard.state, "DASHBOARD_PWD", ""):
            resp = app_client.get("/api/status")
            assert resp.status_code == 200

    def test_passphrase_blocks_html_with_redirect(self, app_client):
        with patch.object(dashboard.state, "DASHBOARD_PWD", "s3cret"):
            resp = app_client.get("/")
            assert resp.status_code == 302
