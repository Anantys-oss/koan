...
# ---------------------------------------------------------------------------
# get_project_mission_hooks (per-project opt-in override)
# ---------------------------------------------------------------------------


class TestGetProjectMissionHooks:
    def test_no_koan_root_returns_none(self, monkeypatch):
        from app.projects_config import get_project_mission_hooks
        monkeypatch.delenv("KOAN_ROOT", raising=False)
        assert get_project_mission_hooks("myapp") is None
    ...
    def test_true_override(self, koan_root, monkeypatch):
        ...
        monkeypatch.setenv("KOAN_ROOT", koan_root)
        assert get_project_mission_hooks("myapp") is True

    def test_false_override(self, koan_root, monkeypatch):
        from app.projects_config import get_project_mission_hooks
        _minimal_config(koan_root, extra="    mission_hooks: false\n")
        monkeypatch.setenv("KOAN_ROOT", koan_root)
        assert get_project_mission_hooks("myapp") is False

    def test_nested_enabled_form(self, koan_root, monkeypatch):
        from app.projects_config import get_project_mission_hooks
        _minimal_config(
            koan_root, extra="    mission_hooks:\n      enabled: true\n")
        monkeypatch.setenv("KOAN_ROOT", koan_root)
        assert get_project_mission_hooks("myapp") is True
