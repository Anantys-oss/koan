...
- **The `effort:` absence contract is the dynamic default.** With no `effort:`
  ...
  the dynamic default. The mission-type axis is what makes a `/review` cheap
  even when it runs in deep budget mode. `get_effort_for_mode()` is the
  type-unaware wrapper and must stay equivalent to `get_effort(mode, "")`.
