...
- **`config_migration.py`** — In-place, idempotent shape migrations for an operator's existing `instance/config.yaml`. Unlike `projects_migration` (which generates a new file), it patches only the lines whose shape changed, so **comments survive** — never round-trip the document through PyYAML. `migrate_mcp_config()` converts a pre-MCP-server `mcp: [paths]` list into `mcp: {configs: [paths]}`; the rewrite is re-parsed and diffed against the original before it is written, and the pre-migration file is backed up to `config.yaml.bak-mcp-mapping`. `run_config_migrations()` runs at startup **before** `validate_config_or_raise` so a config shape change self-heals instead of hard-stopping the agent. Add future shape migrations here.
