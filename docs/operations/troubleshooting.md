...
4. **Forced restart** — `/restart --force` kills the in-flight mission and restarts both daemons immediately, instead of waiting for the mission to end like plain `/restart`. The mission is re-queued to Pending by crash recovery on the next startup. Reach for it when `/abort` doesn't land or the runner ignores a polite restart.
