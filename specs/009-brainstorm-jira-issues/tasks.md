...
- [X] T017 [P] [US3] Tests in `koan/tests/test_brainstorm_jira.py`: mocked Jira tracker asserts N `link_issues` calls (master→each sub) after master creation, and that a raised/False link failure leaves the run successful with all issues intact; mocked GitHub run asserts zero `link_issues` calls and unchanged output.
